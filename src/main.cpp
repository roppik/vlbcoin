...
#if defined(NDEBUG)
# error "VLBcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc847b1e3a70221cc91685f027266deeb17fe5648ca39a796fdfa789e9e31b8e0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // VLBcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "VLBcoin Signed Message:\n";
...
{
    // VLBcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // VLBcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // VLBcoin: 1 days
static const int64 nTargetSpacing = 2.5 * 60; // VLBcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // VLBcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // VLBcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8ac0aa4d3e58e8f92db582c00abd651b157712af8f601839bf897fc283e4cec4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Roppik Slovakia";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043d3f87e5edefb05650f4656d3ad2341be50bf648650fbabf3a7f589c03f2e2ed76e907bf187b8178a7d73c4f9effc89ed9fd92204a6178508b0eb163b5967e7b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1549659916;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085355647;

        if (fTestNet)
        {
            block.nTime    = 1549659891;
            block.nNonce   = 385982636;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa2c94dffd4f56f62141693883d31fbd862d378b4a35436662d93cdc4c8b95610"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc5, 0xb4, 0xda }; // VLBcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VLBcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VLBcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VLBcoinMiner(CWallet *pwallet)
{
    printf("VLBcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vlbcoin-miner");
    ...
    try { loop {
        ...
        printf("Running VLBcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VLBcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VLBcoinMiner, pwallet));
}
